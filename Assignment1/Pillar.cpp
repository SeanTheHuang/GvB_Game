...
CPillar::CPillar(GLuint _shaders, glm::vec3 _position, Level& level) :
	m_iIndices(0),
	m_shaders(_shaders),
	pTexture(nullptr),
...
void CPillar::SetPhysics()
{
	...
	dynamicBox.SetAsBox(1.0f / Level::s_kPixelsPerMeter, 2.0f / Level::s_kPixelsPerMeter);	// The pillars dimentions are 2 by 4 units
...
CPillar * CPillar::CreatePillar(GLuint _shaders, glm::vec3 _position, Level& level)
{
	CPillar* platform = new CPillar(_shaders, _position, level);
	return platform;
}

void CPillar::getUniformLocation()
{
